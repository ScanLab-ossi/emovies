(function (emovis) {
    const famousMovieID = ['1190080', '435761', '1049413', '266543', '1588170', '468536', '167260', '1375666', '1675434', '407887', '482571', '1345836', '1010048', '372784', '499549'];
	var filterMovieID = [];
	/*
...
//                if(element.movieTitle === filters.title){
        ...
        console.log('newFilterData after', newFilteredData);
      if (filters.angerLessThan ||
		...
		if(filters.angerLessThan == null) {filters.angerLessThan=0;}
		if(filters.anticipationLessThan == null) {filters.anticipationLessThan=0;}
		if(filters.joyLessThan == null) {filters.joyLessThan=0;}
		if(filters.trustLessThan == null) {filters.trustLessThan=0;}
		if(filters.fearLessThan == null) {filters.fearLessThan=0;}
		if(filters.surpriseLessThan == null) {filters.surpriseLessThan=0;}
		if(filters.disgustLessThan == null) {filters.disgustLessThan=0;}
		if(filters.sadLessThan == null) {filters.sadLessThan=0;}
          ...
          var filteredData = newFilteredData.filter((element, index, array) => {
              ...
              if((filters.angerLessThan)||(filters.anticipationLessThan)||(filters.joyLessThan)||(filters.trustLessThan)
				||(filters.fearLessThan)||(filters.surpriseLessThan)||(filters.disgustLessThan)||(filters.sadLessThan)){
                  if((element["anger-Percentile"] >= ((filters.angerLessThan))) &&
				  (element["anticipation-Percentile"] >= ((filters.anticipationLessThan)))&&
				  (element["joy-Percentile"] >= ((filters.joyLessThan))) &&
				  (element["trust-Percentile"] >= ((filters.trustLessThan))) &&
				  (element["fear-Percentile"] >= ((filters.fearLessThan))) &&
				  (element["surprise-Percentile"] >= ((filters.surpriseLessThan))) &&
				  (element["disgust-Percentile"] >= ((filters.disgustLessThan))) &&
				  (element["sadness-Percentile"] >= ((filters.sadLessThan))))
				  {
                      returntrue = true;
                  }
              }
...
/*              if(filters.angerLessThan) {
                  if(element["anger-per"] >= ((filters.angerLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              if(filters.anticipationLessThan) {
                  if(element["anticipation-per"] >= ((filters.anticipationLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.joyLessThan) {
                  if(element["joy-per"] >= ((filters.joyLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.trustLessThan) {
                  if(element["trust-per"] >= ((filters.trustLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.fearLessThan) {
                  if(element["fear-per"] >= ((filters.fearLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.surpriseLessThan) {
                  if(element["surprise-per"] >= ((filters.surpriseLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.disgustLessThan) {
                  if(element["disgust-per"] >= ((filters.disgustLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
              }
              ...
              if(filters.sadLessThan) {
                  if(element["sadness-per"] >= ((filters.sadLessThan))){
                      returntrue = true;
                  }
				  else {returntrue = false;}
...
//$('#chart').append(legendglyph);
        ...
        svg = svg.attr("width", width - margin - margin) //g - main container
...
//		var colors_dark = ['#984ea3','#ffff56','#e41a1c','#ff7f00','#ffff33','#4daf4a','#a65628','#f781bf']  //Peter's colors
		var colors_dark = ['#ff7f00','#ffff33','#a65628','#e41a1c','#4daf4a','#984ea3','#f781bf','#377eb8']  //Peter's colors2
...
//		var colors_bright = ['#b3cde3','#decbe4','#ccebc5','#fbb4ae','#fed9a6','#ffffcc','#e5d8bd','#fddaec']   //Peter's colors
		var colors_bright = ['#fed9a6','#ffffcc','#e5d8bd','#fbb4ae','#ccebc5','#decbe4','#fddaec','#b3cde3']   //Peter's colors2
   ...
   d3.selection.prototype.moveToFront = function() {  
      return this.each(function(){
        ...
        var drawGlyph = function (svg, point) {
            ...
            var sizeScale = d3.scaleLinear().range([1, currentSizeCoeff * config.size / 3]).domain([0, 0.15]).clamp(true);
            for (var i = 0; i < 3; i++) {
                for (var j = 0; j < 3; j++) {
                    if (i != 1 || j != 1) {
...
//                        let key_per = emotions_Percentile[counter];
                        ...
                        if (filters === '' || jQuery.inArray(key, filters) > -1) {
                            ...
                            if(point.filtered ) {
                                glyphData.push({
                                    x: xScale(+point.x) + j * sizeScale(+point.Total) * 2,
                                    y: yScale(+point.y) + i * sizeScale(+point.Total) * 2,
                                    size: sizeScale(+point.Total) * 2,
...
//right panel
                    var deatilsContent = '<div class="movie-details md"><div class="movie-details-container md" style="position: relative">';
...
//                            content = content + `<p style="text-transform: capitalize;margin:0; padding: 5px; font-size:12px; line-height: 0.7;"><span style="display: inline-block; width: 6px;height: 6px; margin-right: 5px; border-radius: 50%;background: ${color}"></span>${Key}: <b style="float: right; margin-left: 5px; color: ${color}">${(+point[key]).toFixed(2)} %</b></p>` //key.replace('_',' ').capitalize()
                            content = content + `<p style="text-transform: capitalize;margin:0; padding: 5px; font-size:12px; line-height: 0.7;"><span style="display: inline-block; width: 9px;height: 9px; margin-right: 5px; border-radius: 0%;background: ${color}"></span>${Key}: <b style="float: right; margin-left: 5px; ">${(+point[key]).toFixed(0)} %</b></p>` //key.replace('_',' ').capitalize()
							}
...
//                            deatilsContent = deatilsContent + `<p  class="movie-details-line md"><span class="md">Link to the movie: </span><b class="movie-details-value md"> <a href="http://dddd.com/${point[key]}" target="_blank" style="text-transform: none">http://dddd.com/${point[key]}</a> </b></p>` //key.replace('_',' ').capitalize()
					...
					deatilsContent = deatilsContent + `<p  class="movie-details-line"><span>iMDB Movie link:</span><b class="movie-details-value"><a target="_blank" rel="Movie Emotions imdb" href="https://www.imdb.com/title/tt${movieidlen}">iMDB</a></b></p>`
...
//                })
			...
			glyph.selectAll(".emo").data(glyphData).enter().append("g")			
                .style("stroke-width", function (d) {
					if (d.filtered) {
            ...
            if ( famousMovieID.indexOf(point.movie_id) !== -1 &&  $('#labels')[0].checked) {
                ...
                glyph.selectAll(".emo").data(Poiont).enter().append("text")
                    .text( function (d) {
                        let that = this;
                        setTimeout(function () {
        ...
        var opacityScale1 = d3.scaleLinear()
            .domain([49, 50])
            .range([0.1,1]).clamp(true);

        var perecentScale = d3.scaleLinear()
            .domain([-3, 3])
            .range([0, 100])
            .clamp(true);
        ...
        $('#chart-slider').off('change').on('change', function(){
            var value = $(this).val();
            currentSizeCoeff = value;
            console.log(value);
            // var glyphs = svg.selectAll(".glyph");
